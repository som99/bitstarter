...
var inst_restler = require('restler');
...
if(require.main == module) {
    ...
    if (program.url.length > 0) {
        var urlMatterLoad = function(result) {
            if(result instanceof Error) {
                console.log("Error: url can't be loaded. %s. Exiting.", result.message);
                process.exit(2);
            } else {
        ...
        inst_restler.get(program.url).on('complete', urlMatterLoad);
    } else {
